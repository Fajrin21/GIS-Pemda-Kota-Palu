...
$servername = "localhost";
  ...
  $fetchQuery = "SELECT luas FROM peta";  // Assuming you want to group by kabkota_kua
...
if ($fetchResultNikah->num_rows > 0) {
    ...
    while ($row = $fetchResultNikah->fetch_assoc()) {
        ...
        if (isset($groupedData[$lokasi])) {
            $jumlah_pernikahan = $groupedData[$lokasi] + 1;
        }

        $groupedData[$lokasi] = $jumlah_pernikahan;
    }
}
...
legend.addTo(map);
    ...
    var jsonTest = new L.GeoJSON.AJAX(["./src/html/data/tes.geojson"], { style: style,
